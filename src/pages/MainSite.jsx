...
// ─── SUPABASE CONFIG ──────────────────────────────────────────────────────────
const SUPABASE_URL = 'https://xhqrmuqhpdbuaepkizxl.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhocXJtdXFocGRidWFlcGtpenhsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzgxMjEwOTMsImV4cCI6MjA5MzY5NzA5M30.jIppdRIHFPB4I59PxhZWF4Kg-yqtptvejuFl9NNyfas';
...
function TypewriterHero({ onPosterTrigger }) {
  ...
  return (
    <section className="hero-section">
      ...
      {userTyping && (
        <div className="hero-hint">— type freely · type 'poster' + enter for something_</div>
      )}
      {!userTyping && settled && (
        <div className="hero-hint">— click anywhere and type something_</div>
      )}
...
// ─── INSTAGRAM SECTION ────────────────────────────────────────────────────────
// Drop your photos into src/images/instagram/ named ig-1.jpg through ig-12.jpg
// The section gracefully hides photos that don't exist yet
const IG_COUNT = 12;
const IG_PHOTOS = Array.from({ length: IG_COUNT }, (_, i) => {
  try { return require(`../images/instagram/ig-${i + 1}.jpg`); } catch { return null; }
});
...
function InstagramSection() {
  ...
  useReveal(labelRef, { y: 20, duration: 0.6 });

  useEffect(() => {
    if (!stripRef.current || !sectionRef.current) return;
    // Horizontal scroll driven by vertical scroll — Jasmine Gunarto style
  ...
  const validPhotos = IG_PHOTOS.filter(Boolean);

  // If no photos yet, show a placeholder state
  if (validPhotos.length === 0) {
    return (
      <section className="ig-section-placeholder">
        <div ref={labelRef} className="ig-label-row">
          <span className="ig-label">instagram_</span>
          <a href="https://instagram.com/pseudo.sq" target="_blank" rel="noopener noreferrer" className="ig-handle">
            @pseudo.sq ↗
          </a>
        </div>
  ...
  return (
    <section ref={sectionRef} className="ig-section">
      <div ref={labelRef} className="ig-label-row">
        <span className="ig-label">instagram_</span>
        <a href="https://instagram.com/pseudo.sq" target="_blank" rel="noopener noreferrer" className="ig-handle">
          @pseudo.sq ↗
      ...
      <div ref={stripRef} className="ig-strip">
        {validPhotos.map((src, i) => (
          <div key={i} className="ig-photo-wrap">
            <img src={src} alt={`pseudo.sq ${i + 1}`} className="ig-photo" />
            <div className="ig-photo-overlay">
              <span className="ig-photo-num">{String(i + 1).padStart(2, '0')}_</span>
            </div>
          </div>
        ))}
        {/* End card */}
        <div className="ig-end-card">
          <a href="https://instagram.com/pseudo.sq" target="_blank" rel="noopener noreferrer" className="ig-end-link">
            <span className="ig-end-handle">@pseudo.sq</span>
            <span className="ig-end-sub">view on instagram ↗</span>
          </a>
        </div>
      </div>
    </section>
...
function StickyNote({ note, onDragEnd }) {
  ...
  function onMouseDown(e) {
    ...
    function onUp(e) {
      ...
      const dx = ((e.clientX - drag.current.startX) / window.innerWidth) * 100;
      const dy = ((e.clientY - drag.current.startY) / window.innerHeight) * 100;
      const nx = Math.max(0, Math.min(88, drag.current.origX + dx));
      const ny = Math.max(0, Math.min(82, drag.current.origY + dy));
      onDragEnd(note.id, nx, ny);
  ...
  return (
    <div ref={ref} className={`sticky-note ${note.type === 'prewritten' ? 'prewritten' : ''}`}
...
function StickyBoard({ onClose }) {
  ...
  async function loadNotes() {
    try {
      ...
      setError('could not load notes_');
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleClose() {
    gsap.to(boardRef.current, {
      opacity: 0, y: 20, duration: 0.4, ease: 'power2.in', onComplete: onClose
    });
  }
  ...
  async function addNote() {
    ...
    try {
      ...
      const result = await dbFetch('notes', {
        method: 'POST',
        body: JSON.stringify(newNote),
      });
      if (result) setNotes(prev => [...prev, Array.isArray(result) ? result[0] : result]);
      setFormMsg(''); setFormName(''); setShowForm(false);
    } catch (e) {
      setError('could not post note — try again_');
    } finally {
      setPosting(false);
    }
  }

  async function updateNotePos(id, x, y) {
    // Easter eggs store locally only
    const isEgg = EASTER_EGGS.find(e => e.id === id);
    if (isEgg) {
      setEggPositions(prev => ({ ...prev, [id]: { x, y } }));
      return;
    }
    setNotes(prev => prev.map(n => n.id === id ? { ...n, x, y } : n));
    try {
      await dbFetch(`notes?id=eq.${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ x, y }),
      });
    } catch (e) { /* position update failed silently */ }
  }

  // Merge easter eggs with position overrides
  const eggsWithPos = EASTER_EGGS.map(e => ({
    ...e,
    ...(eggPositions[e.id] || {}),
  }));
  ...
  return (
    <div ref={boardRef} className="board-overlay">
        ...
        <div className="board-form">
          <div className="board-form-inner">
            ...
            <div className="board-form-bottom">
              ...
              />
              <button
                className="board-form-submit"
                onClick={addNote}
                disabled={!formMsg.trim() || !formName.trim() || posting}
              >
      ...
      <div className="board-canvas">
        ...
        {/* Easter eggs */}
        {eggsWithPos.map(note => (
          <StickyNote key={note.id} note={note} onDragEnd={updateNotePos} />
        ))}

        {/* User notes from Supabase */}
        {notes.map(note => (
          <StickyNote key={note.id} note={note} onDragEnd={updateNotePos} />
        ))}
      ...
      <div className="board-footer">
        <span>shared public board · powered by supabase_</span>
        <span>malvinboye@gmail.com</span>
        <span>esc to leave_</span>
      </div>
    </div>
  );
}
...
function CursorMedia({ img, visible }) {
  ...
  useEffect(() => {
    if (!ref.current) return;
    gsap.to(ref.current, { opacity: visible ? 1 : 0, scale: visible ? 1 : 0.9, duration: visible ? 0.3 : 0.18 });
  }, [visible]);
    ...
    <div className="main-site">
      ...
      <header className="top-bar">
        <div className="top-bar-left">
          ...
          <span className="top-time"><LiveClock /></span>
      ...
      <section className="statement-section" id="about">
        <div ref={statementRef} className="statement-text">
          <span className="statement-line">Life.</span>
          <span className="statement-line">People.</span>
          <span className="statement-line">Art.</span>
        </div>
        <p className="statement-sub">— that's what it comes down to.</p>
      </section>

      {/* ORIGIN */}
      <section className="origin-section">
        <div ref={originRef} className="origin-inner">
      ...
      <section className="bio-section">
        <div ref={bioRef} className="bio-inner">
          <div className="bio-num">01_</div>
          <div className="bio-content">
            <p className="bio-text">Someone who loves people, loves new experiences, and loves being put outside of his comfort zone to get to know anything and everything that exists in this world( to the best of his abilities).</p>
            <p className="bio-text">Curious by nature. Creative by instinct. Started drawing around age three. Piano and drums came around the same time. Art is something I hold very dear.</p>
      ...
      <section className="lang-section">
        <div ref={langRef} className="lang-inner">
          ...
          <div className="lang-list">
            ...
            <div className="lang-item">
              ...
              <span className="lang-note">fluent — self-taught through cinema</span>
            </div>
          </div>
          <p className="lang-story">Korean started through my older brother(big ups) he got me into Korean cinema. The dramas came naturally after that. I don't watch things with subtitles unless I really have to. That became the excuse I gave myself to actually learn the language.</p>
        </div>
      </section>

      {/* QUOTE */}
      <section className="quote-section">
        <div ref={quoteRef} className="quote-inner">
      ...
      <section className="yt-section">
        <div ref={ytRef} className="yt-inner">
      ...
      <section id="contact" className="contact-section">
        <div ref={contactRef} className="contact-inner">
          ...
          <p className="contact-note">Open to internships, collaborations, and interesting problems. Based in Washington DC. Originally from Tema, Ghana.</p>
        </div>
